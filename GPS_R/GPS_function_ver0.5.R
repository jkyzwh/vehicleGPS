...
singleDataINI = function(GPSData)
{
  ...
  #增加相邻数据行之间的时间差
  GPSData$GpsTime_diff = c(0,abs(diff(GPSData$GpsTime)))
  # 利用插值法处理GPS无信号导致的经纬度和速度值为零
  GPSData$type = "signal_OK"
  j = 0
  for(i in 1:length(GPSData$vehicleID))
  {
    ...
    {
      #print(i)
      GPSData$type[i] = "signal_No"
      if(i>j)
      {
        for(k in i:(length(GPSData$vehicleID)-i))
        {
          if(GPSData$longitude[k]>0)
          {
            zero_length = k-i+1
            longitude_add = (GPSData$longitude[k]-GPSData$longitude[i-1])/zero_length #经度
            latitude_add = (GPSData$latitude[k]-GPSData$latitude[i-1])/zero_length #维度
            speed_add = (GPSData$GPS_Speed[k]-GPSData$GPS_Speed[i-1])/zero_length  #速度
            elevation_add = (GPSData$elevation[k]-GPSData$elevation[i-1])/zero_length #高程
            j = k
            #print(k)
            break()
          }
    ...
    if(i<j)
    {
      GPSData$longitude[i] = GPSData$longitude[i-1]+longitude_add
      GPSData$latitude[i] = GPSData$latitude[i-1]+latitude_add
      GPSData$GPS_Speed[i] = GPSData$GPS_Speed[i-1]+speed_add
      GPSData$elevation[i] = GPSData$elevation[i-1]+elevation_add
    }
  }
