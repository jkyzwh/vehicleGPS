...
#==============================================================================
def vehicleDataINI(GPSData_initial,ID):    
     GPSData = GPSData_initial.loc[GPSData_initial['vehicleID'] == ID].copy()
...
for i in range(len(ID.index)):
    IDn = ID.iloc[i]
    GPSData = vehicleDataINI(GPSData_initial,IDn)
    GPSData = GPSData.loc[GPSData['GPS_Speed'] > 0]
    if len(GPSData['GPS_Speed'])>5:
